...
-- 테스트용 시퀀스 삭제 (H2 문법)
DROP SEQUENCE IF EXISTS seq_reply_id;
DROP SEQUENCE IF EXISTS seq_board_id;
DROP SEQUENCE IF EXISTS seq_member_id;
DROP SEQUENCE IF EXISTS seq_uploadfile_id;
DROP SEQUENCE IF EXISTS seq_product_id;
DROP SEQUENCE IF EXISTS seq_code_id;
...
CREATE TABLE replies(
    ...
    rindent        NUMBER(3)      DEFAULT 0,        -- 댓글 들여쓰기
    status         CHAR(1)        DEFAULT 'A',      -- 댓글 상태 (A: 활성, D: 삭제)
    cdate          TIMESTAMP      DEFAULT CURRENT_TIMESTAMP, -- 생성일시
    udate          TIMESTAMP      DEFAULT CURRENT_TIMESTAMP, -- 수정일시
    ...
    CONSTRAINT ck_replies_status CHECK (status IN ('A','D'))
);
