package com.kh.demo.admin.product.page;

import com.kh.demo.admin.product.page.form.DetailForm;
import com.kh.demo.admin.product.page.form.SaveForm;
import com.kh.demo.admin.product.page.form.UpdateForm;
...
public class AdminProductController extends BaseController {
    ...
    public String add(@Valid @ModelAttribute SaveForm saveForm,
        ...
        if (imageFiles != null && !imageFiles.isEmpty()) {
            for (MultipartFile file : imageFiles) {
                if (file != null && !file.isEmpty()) {
                    FileUploadService.ValidationResult result = 
                        fileUploadService.validateFileWithDetails(file, 
                            fileUploadService.getAllowedExtensions("image"), 
                            fileUploadService.getMaxFileSize("image"));
                    
                    if (!result.isValid()) {
                        validationErrors.add("이미지 파일: " + result.getMessage());
                    }
                }
            }
        }
        ...
        if (manualFiles != null && !manualFiles.isEmpty()) {
            for (MultipartFile file : manualFiles) {
                if (file != null && !file.isEmpty()) {
                    FileUploadService.ValidationResult result = 
                        fileUploadService.validateFileWithDetails(file, 
                            fileUploadService.getAllowedExtensions("manual"), 
                            fileUploadService.getMaxFileSize("manual"));
                    
                    if (!result.isValid()) {
                        validationErrors.add("설명서 파일: " + result.getMessage());
                    }
                }
            }
        }
        ...
        if (!validationErrors.isEmpty()) {
            log.warn("파일 유효성 검사 실패: {}", validationErrors);
            
            // 검증 실패한 파일들을 모델에 추가하여 사용자에게 표시
            if (imageFiles != null && !imageFiles.isEmpty()) {
            ...
            // 유효성 검사 오류 메시지를 모델에 추가
            model.addAttribute("fileValidationErrors", validationErrors);
        ...
        try {
            ...
            products.setCategoryId( saveForm.getCategoryId());
    ...
    public String detail(@PathVariable Long productId, Model model, HttpSession session) {
        ...
        // Long categoryId를 decode로 변환
        String categoryName = codeSVC.getCodeDecode("PRODUCT_CATEGORY", product.getCategoryId());
        detailForm.setCategoryName(categoryName);
    ...
    public String edit(@PathVariable Long productId,
        ...
        try {
            ...
            products.setCategoryId(updateForm.getCategoryId());
