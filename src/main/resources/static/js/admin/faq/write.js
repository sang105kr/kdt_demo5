/**
 * 관리자 FAQ 작성 페이지 JavaScript
 */

// DOM 로드 완료 후 실행
document.addEventListener('DOMContentLoaded', function() {
    console.log('관리자 FAQ 작성 페이지 로드됨');
 ...
 * FAQ 작성 폼 초기화
 */
function initFaqWriteForm() {
    const form = document.getElementById('faqWriteForm');
    
    if (!form) {
        console.error('FAQ 작성 폼을 찾을 수 없습니다.');
        return;
    }
...
async function submitFaqForm(form) {
    ...
    try {
        ...
        if (typeof ajax !== 'undefined' && ajax.post) {
            const result = await ajax.post('/admin/faq/write', formData);
            
            if (result && result.code === '00') {
                if (typeof showToast === 'function') {
                    showToast('FAQ가 성공적으로 작성되었습니다.', 'success');
                } else {
                ...
                throw new Error(result?.message || 'FAQ 작성에 실패했습니다.');
            }
        } else {
        ...
        console.error('FAQ 작성 오류:', error);
        if (typeof showToast === 'function') {
            showToast(error.message || 'FAQ 작성에 실패했습니다.', 'error');
        } else {
            alert(error.message || 'FAQ 작성에 실패했습니다.');
        }
    } finally {
        hideLoading();
    }
...
function hideLoading() {
    ...
    if (submitBtn) {
        ...
        submitBtn.innerHTML = '<i class="fas fa-save"></i> FAQ 작성';
    }
}
