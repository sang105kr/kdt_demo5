...
class ReplySPA {
        ...
        return `
            <div class="reply-form">
                <form id="replyForm">
                    ...
                    <div class="btn-area">
                        <button type="button" class="btn btn--primary" id="submitReplyBtn">댓글</button>
                    </div>
                </form>
            </div>
        `;
    ...
    bindEvents() {
        ...
        document.addEventListener('click', (e) => {
            if (!e.target.closest('.reply-more-menu')) {
    ...
    createReplyElement(reply) {
        ...
        replyDiv.innerHTML = `
            ...
            <div class="reply-content" data-original-content="${this.escapeHtml(reply.rcontent)}">
                ${this.escapeHtml(reply.rcontent)}
            </div>
            <div class="reply-actions">
                ...
                ` : ''}
                ${isAuthor ? `
                    <div class="reply-more-menu">
                        <button type="button" class="btn btn--small btn--outline more-menu-btn" data-reply-id="${reply.replyId}">
                            <i class="fas fa-ellipsis-h"></i>
                        </button>
                        <div class="more-menu-dropdown" style="display: none;">
                            <button type="button" class="more-menu-item edit-reply-btn" data-reply-id="${reply.replyId}">
                                <i class="fas fa-edit"></i> 수정
                            </button>
                            <button type="button" class="more-menu-item delete-reply-btn" data-reply-id="${reply.replyId}">
            ...
            <div class="reply-reply-form" style="display: none;">
                <form onsubmit="return false;">
                    ...
                    <div class="btn-area">
                        <button type="button" class="btn btn--small btn--primary submit-reply-reply-btn">답글</button>
    ...
    bindReplyEvents(replyDiv, reply) {
        ...
        // 더보기 메뉴 버튼
        const moreMenuBtn = replyDiv.querySelector('.more-menu-btn');
        if (moreMenuBtn) {
            moreMenuBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                this.toggleMoreMenu(reply.replyId);
            });
        }
        ...
        if (editReplyBtn) {
            editReplyBtn.addEventListener('click', () => {
                this.startInlineEdit(reply.replyId, reply.rcontent);
                this.hideAllMoreMenus();
            });
        }
    ...
    startInlineEdit(replyId, currentContent) {
        const replyElement = document.querySelector(`[data-reply-id="${replyId}"]`);
        ...
        contentElement.innerHTML = `
            <textarea class="inline-edit-textarea" placeholder="댓글을 수정하세요..." required>${this.escapeHtml(currentContent)}</textarea>
            <div class="inline-edit-actions">
                <button type="button" class="btn btn--small btn--primary submit-inline-edit-btn">완료</button>
                <button type="button" class="btn btn--small btn--outline cancel-inline-edit-btn">취소</button>
            </div>
        `;
        ...
        // 이벤트 바인딩
        const submitBtn = contentElement.querySelector('.submit-inline-edit-btn');
        const cancelBtn = contentElement.querySelector('.cancel-inline-edit-btn');
        
        submitBtn.addEventListener('click', () => {
            this.submitInlineEdit(replyId, contentElement);
        });
        
        cancelBtn.addEventListener('click', () => {
            this.cancelInlineEdit(contentElement);
        });
        
        // Enter 키로 완료, Esc 키로 취소
    ...
    submitInlineEdit(replyId, contentElement) {
        const textarea = contentElement.querySelector('.inline-edit-textarea');
        ...
        this.submitEditReply(replyId, newContent, contentElement);
    }
    
    cancelInlineEdit(contentElement) {
        const originalContent = contentElement.dataset.originalContent;
        const replyElement = contentElement.closest('.reply-item');
        const actionsElement = replyElement.querySelector('.reply-actions');
    ...
    cancelAllInlineEdit() {
        document.querySelectorAll('.inline-edit-textarea').forEach(textarea => {
            const contentElement = textarea.closest('.reply-content');
            if (contentElement) {
                this.cancelInlineEdit(contentElement);
            }
        });
    ...
    toggleMoreMenu(replyId) {
        const replyElement = document.querySelector(`[data-reply-id="${replyId}"]`);
        const dropdown = replyElement.querySelector('.more-menu-dropdown');
    ...
    async submitEditReply(replyId, newContent, contentElement) {
        try {
            console.log('댓글 수정 요청 URL:', '/api/replies');
            console.log('댓글 수정 요청 데이터:', { replyId, rcontent: newContent });
            ...
            if (response && (response.code === 'SUCCESS' || response.code === '00')) {
                // 인라인 편집 완료
                this.finishInlineEdit(contentElement, newContent);
    ...
    finishInlineEdit(contentElement, newContent) {
        const replyElement = contentElement.closest('.reply-item');
        const actionsElement = replyElement.querySelector('.reply-actions');
